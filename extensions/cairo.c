...
static inline unsigned char conv_rgbafloat_cairo32_map(float value, float alpha)
{
...
static inline unsigned char conv_rgbafloat_cairo24_map(float value)
{
  unsigned short index;
...
static long
conv_rgbAfloat_cairo32_le (unsigned char *src_char, unsigned char *dst, long samples)
{
  long n = samples;
  ...
  while (n--)
    {
      ...
      else
      {
        ...
        if (src[3] >= 1.0)
          {
            dst[0] = conv_rgbafloat_cairo32_map(src[2] * recip, 1.0f);
            dst[1] = conv_rgbafloat_cairo32_map(src[1] * recip, 1.0f);
            dst[2] = conv_rgbafloat_cairo32_map(src[0] * recip, 1.0f);
            dst[3] = 0xFF;
          }
        else
          {
            dst[0] = conv_rgbafloat_cairo32_map(src[2] * recip, alpha);
            dst[1] = conv_rgbafloat_cairo32_map(src[1] * recip, alpha);
            dst[2] = conv_rgbafloat_cairo32_map(src[0] * recip, alpha);
...
static long
conv_rgbfloat_cairo24_le (unsigned char *src_char, unsigned char *dst, long samples)
{
  long n = samples;
  float *src  = (float*)src_char;
  ...
  while (n--)
    {
      dst[0] = conv_rgbafloat_cairo24_map(src[2]);
      dst[1] = conv_rgbafloat_cairo24_map(src[1]);
      dst[2] = conv_rgbafloat_cairo24_map(src[0]);
...
static long
conv_rgbafloat_cairo24_le (unsigned char *src_char, unsigned char *dst, long samples)
{
  long n = samples;
  float *src  = (float*)src_char;
  ...
  if (littleendian)
    {
      ...
      babl_conversion_new (babl_format ("RGBA float"), f24, "linear",
                           conv_rgbafloat_cairo24_le, NULL);
      babl_conversion_new (babl_format ("RGB float"), f24, "linear",
                           conv_rgbfloat_cairo24_le, NULL);
    }
  else
    {
      babl_format_new (
